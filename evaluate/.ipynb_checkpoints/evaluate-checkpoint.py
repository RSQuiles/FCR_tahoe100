...
def evaluate_prediction_baseline(adata, batch_size=None, min_samples=30):
    ...
    for pert_category in np.unique(adata.obs.cov_pert_name):
       
        idx = np.where(adata.obs.cov_pert_name == pert_category)[0]

        # estimate metrics only for reasonably-sized perturbation/cell-type combos
        if len(idx) > min_samples:

            yt = adata.X[idx, :]
            out = adata.obsm["X_normalized_scVI"].iloc[idx, :].to_numpy()
            yp = out
            yp_m = np.squeeze(yp.mean(axis=0))
            yt_m = np.squeeze(np.array(yt.mean(axis=0)))
            mean_score.append(r2_score(yt_m, yp_m))
            mean_score_mean = np.mean(mean_score) if len(mean_score) else -1
            mean_score_stddev = np.std(mean_score) if len(mean_score) else -1
