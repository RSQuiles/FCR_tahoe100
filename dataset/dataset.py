...
class Dataset:
    def __init__(
        ...
        max_size: int = None, # NEW: to handle huge datasets (load only a subset of data if too big, mainly for plotting purposes)
    ):
        ...
        if max_size is not None and self.adata.n_obs > max_size:
            ...
            indices = np.random.choice(self.adata.n_obs, max_size, replace=False)

            # Efficient way: only read what you need
            print("Subsetting...")
            subset_X = self.adata.X[indices]
            self.adata = sc.AnnData(
                X=subset_X,
                obs=self.adata.obs.iloc[indices].copy(),
        ...
        self.control_names = np.unique(self.adata[self.adata.obs[control_key] == 1].obs[perturbation_key])
        ...
        if covariate_keys is not None:
            ...
            self.cov_names_unique = np.unique(self.cov_names)
        else:
            self.cov_names = np.array([""] * len(self.adata))
        ...
        self.cf_genemap = {} # map from covariate names to genes
        cf_pert_dose_name = self.control_names[0]
        control_vals = '1'

        for covariate_name in self.cov_names_unique:
...
def prepare_dataset(args, data_path, split_name, state_dict=None, max_size = 500_000):
    ...
    return_dataset=True, # always return dataset object in this case
    )
