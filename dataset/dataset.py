...
class Dataset:
    def __init__(
        ...
        if type(data) == str:
            print(f"Reading AnnData from {data}...")
            data = sc.read(data)
        ...
        if split_key is None or split_key not in data.obs.columns:
...
def prepare_dataset(args, data_path, split_name, state_dict=None):
    """
